...
Route::middleware(['auth:sanctum','active'])->prefix('kot')->group(function () {
    ...
    Route::post('/{id}/print', [KOTController::class, 'print'])->middleware('role:admin,manager,kitchen_staff,waiter');
    Route::post('/{id}/void', [KOTController::class, 'void'])->middleware('role:admin,manager');
    Route::post('/{id}/reprint', [KOTController::class, 'reprint'])->middleware('role:admin,manager,kitchen_staff,waiter');
