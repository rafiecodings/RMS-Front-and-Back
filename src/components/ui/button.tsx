  ...
  "group/button inline-flex shrink-0 items-center justify-center rounded-xl border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 shadow-button hover:shadow-button-hover [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      ...
      size: {
        ...
        lg: "h-10 gap-2.5 px-5 text-base has-data-[icon=inline-end]:pr-4 has-data-[icon=inline-start]:pl-4",
        icon: "size-9",
        "icon-xs":
...
function isLinkElement(renderProp: unknown): boolean {
  if (!isValidElement(renderProp)) return false
  const { type, props: elementProps } = renderProp as {
    type: unknown
    props: Record<string, unknown>
  }
  if (typeof type === "string") return type === "a"
  if (elementProps && typeof elementProps === "object" && "href" in elementProps) return true
  return false
}
