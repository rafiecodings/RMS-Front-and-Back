...
export function StockAdjustDialog({ open, onOpenChange, onSubmit, isLoading }: StockAdjustDialogProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.ingredient_id || form.quantity <= 0) return;
    onSubmit(form);
    onOpenChange(false);
    setForm({ ingredient_id: "", type: "in", quantity: 0, notes: "" });
  }
      ...
      <div className="bg-background rounded-lg p-6 shadow-lg max-w-md w-full mx-4 relative z-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button type="submit" disabled={isLoading || !form.ingredient_id || form.quantity <= 0}>
